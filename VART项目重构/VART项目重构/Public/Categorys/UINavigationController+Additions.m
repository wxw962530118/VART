...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
   ...
   //某些控制器 不添加手势
    //取出 CancelSideslip.plist文件的Controller 进行判断
    NSString * plistPath = [[NSBundle mainBundle] pathForResource:ControllerCancelSideSlip ofType:@"plist"];
    NSArray  * cancelSideslipArray = [[NSArray alloc] initWithContentsOfFile:plistPath];
    for (NSDictionary * item in cancelSideslipArray) {
//        for (UIViewController * vc in self.navigationController.viewControllers) {
//            if ([vc isKindOfClass:NSClassFromString(item[@"controller_name"])]){
//                if ([item[@"has_tip"] intValue]) {
//                    [((VTBaseViewController *)vc) back];
//                }
//                return NO;
//            }
//        }
